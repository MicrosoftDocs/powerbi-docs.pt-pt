...
|[Abrir e explorar um elemento visual nas Perguntas e Respostas (se as Perguntas e Respostas tiverem sido utilizadas para criar o elemento visual)](end-user-q-and-a.md)     |   sim      |   na      |  na  |
|[Pedir ao Power BI para procurar factos interessantes ou tendências](end-user-insights.md) nos dados do elemento visual.  Estes são denominados *informações automáticas*.     |    sim     |   sim      | não   |
|[Ver apenas um elemento visual de cada vez, através do *modo* de detalhe](end-user-focus.md).     | sim        |   sim      | na  |
|Procurar saber quando é que o elemento visual foi atualizado pela última vez.     |  sim       |    sim     | na  |
|[Ver apenas um elemento visual de cada vez, sem limites ou barras de navegação, através do *modo de* ecrã inteiro](end-user-focus.md).     |   sim      |  sim       | não  |
|[Imprima](end-user-print.md).     |  sim       |   sim      | não  |
|[Examinar o elemento visual ao adicionar e modificar os filtros.](end-user-report-filter.md)     |    não     |   sim      | sim  |
...
| Analisar (consoante o tipo de elemento visual). | não  | sim  | não  |
