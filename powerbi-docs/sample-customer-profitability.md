...
O pacote de conteúdos Exemplo de Lucro do Cliente contém um dashboard, um relatório e um conjunto de dados para uma empresa que fabrica materiais de marketing. Este dashboard foi criado por uma diretora financeira para ver as principais métricas sobre os seus cinco gestores (executivos) de unidade de negócio, produtos, clientes e margens brutas (GM). Desta forma, pode ver rapidamente que fatores têm impacto na rentabilidade.

![Dashboard do Power BI](media/sample-customer-profitability/power-bi-dash.png)

Este exemplo faz parte de uma série de exemplos que ilustra como o Power BI pode ser utilizado com dados, relatórios e dashboards voltados para os negócios. Este exemplo foi criado com dados reais anónimos da obviEnce ([www.obvience.com](http://www.obvience.com/)). Os dados estão disponíveis em vários formatos: pacote/aplicação de conteúdos, livro do Excel ou ficheiro .pbix do Power BI Desktop. Veja [Samples for Power BI](sample-datasets.md) (Exemplos para o Power BI).

## <a name="prerequisites"></a>Pré-requisitos
Quer acompanhar? Este tutorial utiliza o serviço Power BI e o pacote de conteúdos Exemplo de Rentabilidade do Cliente. Uma vez que as experiências do relatório são muito semelhantes, também pode acompanhar com o Power BI Desktop e o ficheiro .pbix de exemplo. Na secção seguinte, encontram-se instruções para se ligar ao pacote de conteúdos e ao ficheiro .pbix.
...
Em alternativa, pode transferir o [Exemplo de Rentabilidade do Cliente](http://download.microsoft.com/download/6/A/9/6A93FD6E-CBA5-40BD-B42E-4DCAE8CDD059/Customer%20Profitability%20Sample%20PBIX.pbix) como um ficheiro .pbix, que foi concebido para utilização com o Power BI Desktop.
...
Vejamos os dois membros da equipa em detalhes e que informações podem ser obtidas. Na caixa **Executive** (Executivos) à esquerda, selecione o nome de Andrew para filtrar a página do relatório e ver apenas os seus dados.
...
Agora, com Valery. Na caixa **Executive** (Executivos), selecione o nome de Valery para filtrar a página do relatório e ver apenas os seus dados. 
...
* Selecionar os outros quadrados **Total Revenue by Region** (Receita Total por Região) mostra-nos dados interessantes: a % de GM varia entre 23% e 79%. Os seus números de receita, em todas as regiões, exceto na região norte, são extremamente sazonais.

Continue a explorar para descobrir porque é que a área de Valery não apresenta um bom desempenho. Analise as regiões, as outras unidades de negócios e a próxima página do relatório: **Análise de Margem do Setor**.

### <a name="industry-margin-analysis"></a>Análise de Margem da Indústria
Esta página de relatório fornece uma secção diferente dos dados. Examina a margem bruta para todo o setor, dividido por segmento. O diretor financeiro utiliza essa página para comparar as métricas de unidade da empresa e comercial para métricas do setor para ajudar a explicar tendências e lucro. Poderá estar a perguntar-se porque é que o gráfico **Gross Margin % by Month and Executive** (Margem Bruta por Mês e Executivo) está nesta página, uma vez que é específico de uma equipa. Tê-lo aqui permite-nos filtrar a página pelo gerente da unidade de negócios.  

![Página do relatório de análise de margem do setor](media/sample-customer-profitability/customer6.png)

Como varia o lucro por setor? Como se os produtos e clientes dividem por setor? Para responder a estas perguntas, selecione um ou mais setores no canto superior esquerdo (comece com o setor de CPG). Para limpar o filtro, selecione o ícone de borracha.

No gráfico de bolhas (**Revenue Var % to Budget, GM%, and RevenueTY by Industry**) (% de Variação da Receita do Orçamento, % de GM e Total da Receita do Ano por Setor), a diretora financeira procura as bolhas maiores, uma vez que têm o maior impacto na receita. Para ver facilmente o impacto de cada gestor por segmento do setor, filtre a página ao clicar no nome de cada gestor no gráfico de área.
...
Esta página está formatada como uma página de resposta da Cortana. Para obter mais informações, veja [Criar uma página de resposta personalizada da Cortana](service-cortana-answer-cards.md).
...
1. Abra o relatório na Vista de edição ao selecionar **Editar relatório**. A vista de Edição só está disponível se for proprietário do relatório. Esta vista é por vezes denominada modo de **criador**. Se, em vez disso, este relatório só tiver sido partilhado consigo, não poderá abri-lo na vista de Edição.
...
3. Escreva *receita total por setor de Valery Ushakov*. Veja como a visualização é atualizada à medida que escreve a pergunta.
...
6. Para ver a receita real, utilize as Perguntas e Respostas para introduzir *receita total por executivo da distribuição por cenário*.  
...
Este ambiente é seguro para fazer experiências. Pode sempre optar por não guardar as alterações. No entanto, se as guardar, pode sempre aceder a **Obter Dados** para obter uma nova cópia deste exemplo.
