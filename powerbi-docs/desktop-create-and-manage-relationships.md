...
Por predefinição, o Power BI Desktop configura automaticamente as propriedades Cardinalidade (direção), Direção de filtro cruzado e Ativo para a sua nova relação; no entanto, pode alterar as definições, se necessário. Para obter mais informações, veja a secção Noções básicas sobre opções adicionais, mais adiante neste artigo.

Verá um erro a indicar que *Uma das colunas tem de ter valores exclusivos* se nenhuma das tabelas selecionadas para a relação tiver valores exclusivos. Pelo menos uma tabela numa relação *tem* de ter uma lista distinta e exclusiva de valores-chave, que é um requisito comum para todas as tecnologias de bases de dados relacionais. 
...
Para obter mais informações, veja a [mensagem de blogue](https://blogs.technet.microsoft.com/cansql/2016/12/19/relationships-in-power-bi-fixing-one-of-the-columns-must-have-unique-values-error-message/).
...
Quando cria ou edita uma relação, pode configurar opções adicionais. Por predefinição, as opções adicionais são automaticamente configuradas com base numa melhor estimativa, que pode ser diferente para cada relação com base nos dados nas colunas.

## <a name="cardinality"></a>Cardinalidade
**Muitos para um (\*:1)** - O tipo predefinido mais comum, o que significa que a coluna numa tabela pode ter mais de uma instância de um valor, enquanto que a outra tabela relacionada, geralmente conhecida como Tabela de referência, tem apenas uma instância de um valor.
...
**Relações muitos para muitos**: Com os modelos compostos, pode estabelecer relações muitos para muitos entre tabelas, o que remove os requisitos para valores exclusivos em tabelas. Também remove soluções anteriores como, por exemplo, apresentar novas tabelas apenas para estabelecer relações. Para obter informações mais detalhadas, veja [Relações com uma cardinalidade de muitos-para-muitos](https://docs.microsoft.com/power-bi/desktop-many-to-many-relationships). 
...
2. Em **Gerir Relações**, clique em **Novo** para abrir a caixa de diálogo **Criar Relação**, onde podemos selecionar as tabelas, colunas e quaisquer definições adicionais que queremos para a relação.
...
Quando uma relação é criada, seja com a Deteção Automática ou manualmente, o Power BI Desktop configura opções adicionais automaticamente com base nos dados das tabelas. Pode configurar estas propriedades de relação adicionais na parte inferior da caixa de diálogo Criar/Editar relação.
...
A primeira opção é *Importar relações de origens de dados* e está selecionada por predefinição. Quando selecionado, o Power BI verifica as relações definidas na sua origem de dados, como relações de chave de referência/chave primária no seu armazém de dados. Se essas relações existirem, serão espelhadas no modelo de dados Power BI quando carregar os dados inicialmente. Esta opção permite que comece rapidamente a trabalhar com o seu modelo, em vez de exigir que encontre ou defina essas relações por si.

A segunda opção é *Atualizar ou eliminar relações quando atualizar dados* e está desativada por predefinição. Se selecionada (ativada ao marcar a caixa ao lado da opção), o Power BI verifica se existem alterações nas relações de origens de dados quando o seu conjunto de dados é atualizado. Se essas relações forem alteradas ou removidas, o Power BI espelhará essas alterações no seu próprio modelo de dados, atualizando-as ou eliminando-as em conformidade.

> [!WARNING]
> Se estiver a utilizar uma segurança ao nível de linha que se baseia nas relações definidas, não recomendamos que selecione a segunda opção, *Atualizar ou eliminar relações quando atualizar dados*. Se uma relação na qual as suas definições de RLS dependam for removida, o seu modelo poderá tornar-se menos seguro. 

A terceira opção é *Detetar automaticamente novas relações depois de os dados serem carregados*, que é descrita na secção [Detetar automaticamente durante carregamento](#autodetect-during-load), que se encontra numa parte anterior deste artigo. 
