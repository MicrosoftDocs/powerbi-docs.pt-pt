...
1. Transfira o [ficheiro PBIX de exemplo de Análise de Revenda](https://download.microsoft.com/download/9/6/D/96DDC2FF-2568-491D-AAFA-AFDD6F763AE3/Retail%20Analysis%20Sample%20PBIX.pbix).

1. Na barra de menus do Power BI Desktop, selecione **Ficheiro** > **Abrir**.
...
1. Com a nova segmentação de dados selecionada, no painel **Campos**, selecione **Distrito** > **GD** para preencher a segmentação de dados. 
    ...
    ![Segmentação de dados com nomes de gestores distritais](media/power-bi-visualization-slicers/power-bi-new-slicer.png)
...
**Criar uma nova segmentação de dados para filtrar dados por intervalo de datas**

1. Selecione a página **Descrição Geral** do relatório. Sem nada selecionado na tela de relatório, no painel **Campos**, selecione **Loja** >  **OpenDate**.

    Esta ação preenche a caixa **Valores** no painel **Visualizações** para criar uma nova visualização.

1. Com a nova visualização selecionada no relatório, selecione o ícone da **Segmentação de Dados** **ícone da segmentação de dados** no painel Visualizações para converter a nova visualização numa segmentação de dados. Esta segmentação de dados **OpenDate** é um controlo de deslize com o intervalo de datas preenchido.
    
    ![Criar visualização OpenDate](media/power-bi-visualization-slicers/power-bi-date-slicer.png)

1. Redimensione e arraste a segmentação de dados e os outros elementos na tela para criar espaço para a segmentação de dados. Apesar de o controlo de deslize ser redimensionado com o tamanho da segmentação de dados, irá desaparecer e as datas serão cortadas se redimensionar a segmentação de dados demasiado pequena. 

1. Selecione diferentes intervalos de data com o controlo de deslize ou selecione um campo de data para introduzir uma data ou apresentar um calendário para uma seleção mais precisa. Note os efeitos nas outras visualizações na página.
...
1. Para alterar o tipo de segmentação de dados, com a segmentação de dados selecionada, paire o cursor do rato sobre a área superior direita da segmentação de dados, selecione o ícone do acento circunflexo que aparece e selecione uma das opções, tais como **Lista** ou **Antes**. Observe como muda o aspeto da segmentação de dados e das opções de seleção. 
 
    ![Novo intervalo para segmentação de dados](media/power-bi-visualization-slicers/power-bi-between-slicer.png)
...
1. No menu do Power BI Desktop, selecione o menu **Formato** em **Ferramentas Visuais** e, em seguida, selecione **Editar interações**.
   ...
   Os controlos de filtro ![controlos de filtro](media/power-bi-visualization-slicers/filter-controls.png), cada um com um **Filtro** e uma opção **Nenhum**, aparecem acima dos elementos visuais na página. Inicialmente, a opção **Filtro** está pré-selecionada em todos os controlos.
...
1. No Power BI Desktop, no menu **Ver**, selecione **Segmentação de dados de sincronização**.
    ...
    ![Seleção de segmentação de dados de sincronização](media/power-bi-visualization-slicers/power-bi-slicer-view-sync.png)

    O painel **Sincronizar segmentação de dados** é apresentado entre os painéis **Filtros** e **Visualizações**.

    ![Painel de segmentação de dados de sincronização](media/power-bi-visualization-slicers/power-bi-slicer-sync-pane.png)
...
1. Na página **Vendas Mensais Distritais** do relatório, selecione a segmentação de dados **Gestor Distrital**. 
    ...
    ![Segmentação de dados de sincronização de Vendas Mensais Distritais](media/power-bi-visualization-slicers/9-sync-slicers.png)
...
1. Na coluna **Visível**, do painel **Segmentação de dados de sincronização**, selecione a página **Novas Lojas**. 
    ...
    ![Selecionar páginas em Segmentação de dados de sincronização](media/power-bi-visualization-slicers/power-bi-sync-slicer-finished.png)
...
1. **Ative** o esquema **Reativo** para alterar o tamanho e a disposição dos itens da segmentação de dados, de acordo com o ecrã da vista e o tamanho da segmentação de dados. 
    ...
    ![Definir esquema reativo](media/power-bi-visualization-slicers/5-responsive.png)
...
1. Em **Controlos de seleção**, ative a **opção Mostrar "Selecionar tudo"** para **Ativada** para adicionar um item **Selecionar Tudo** à segmentação de dados. 
    ...
    **A opção "Selecionar Tudo"** está **Desativada**, por predefinição. Quando está ativada, esta opção, alternada, seleciona ou desseleciona todos os itens. Se selecionar todos os itens, selecionar um item anula a sua seleção, permitindo um tipo de filtro *não é*.
    ...
    ![Controlos de seleção](media/power-bi-visualization-slicers/7-select-all.png)
...
**Título** está **Ativado** por predefinição. Esta seleção apresenta o nome do campo de dados na parte superior da segmentação de dados. 
- Para este tutorial, formate o texto do título da seguinte forma: 
...
### <a name="items-options-list-slicers-only"></a>Opções de itens (apenas segmentações de dados de lista)
1. Para este tutorial, formate as opções de **Itens** da seguinte forma:
...
1. Para **Contorno**, selecione **Moldura** para desenhar um limite à volta de cada item com o tamanho e a cor que definiu nas opções em **Geral**. 
    ...
    ![Opções de contorno de moldura](media/power-bi-visualization-slicers/8-formatted.png)
    
    >[!TIP]
    >- Com a **Orientação** > **Geral** > **Horizontal** selecionada, os itens desselecionados mostram as cores de texto e fundo escolhidas, enquanto os itens selecionados utilizam a predefinição do sistema, normalmente fundo preto com texto branco.
