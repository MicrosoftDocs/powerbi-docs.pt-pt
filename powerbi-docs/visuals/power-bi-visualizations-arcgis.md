...
O exemplo abaixo utiliza uma tela em cinzento escuro para mostrar as vendas regionais como um mapa térmico, em relação a uma camada demográfica do rendimento disponível médio de 2016. Como poderá constatar nesta leitura, a utilização do ArcGIS Maps for Power BI oferece uma capacidade de mapeamento avançado, dados demográficos e visualizações de mapas ainda mais apelativas para contar a melhor história.

![Imagem de abertura do ArcGIS](media/power-bi-visualizations-arcgis/arcgis-tutorial-01.png)

> [!TIP]
> Visite a [**página da Esri sobre o ArcGIS Maps for Power BI**](https://www.esri.com/powerbi) para ver muitos exemplos e ler testemunhos. Além disso, veja a ajuda do **[ArcGIS Maps for Power BI online](https://doc.arcgis.com/en/maps-for-powerbi/get-started/about-maps-for-power-bi.htm) da Esri**.

## <a name="user-consent"></a>Consentimento do utilizador

O ArcGIS Maps for Power BI é disponibilizado pela Esri ([https://www.esri.com](https://www.esri.com/)). A utilização do ArcGIS Maps for Power BI está sujeita aos [termos](https://go.microsoft.com/fwlink/?LinkID=826322) e à [política de privacidade](https://go.microsoft.com/fwlink/?LinkID=826323) da Esri. Os utilizadores do Power BI que queiram utilizar os elementos visuais do ArcGIS Maps for Power BI têm de clicar em Aceitar na caixa de diálogo de consentimento. Esta caixa de diálogo aparece apenas na primeira vez que utilizar o ArcGIS Maps for Power BI.
...
2. O Power BI adiciona um modelo vazio à tela de relatórios. Nesta descrição geral, utilizaremos a versão **Standard** incluída com o Power BI. Se iniciar sessão numa conta válida do ArGIS com o licenciamento adequado, terá acesso a mais funcionalidades. Para obter detalhes, veja a [ajuda do ArcGIS Maps for Power BI online](https://doc.arcgis.com/en/maps-for-powerbi/get-started/account-types.htm).
...
1. No painel Visualizações, clique no separador **Formatar** para ver as opções de formatação.
    ...
    Opções de formatação do ArcGIS Maps:
...
### <a name="edit-an-arcgis-maps-for-power-bi-map"></a>Editar o mapa do ArcGIS Maps for Power BI

Depois de adicionar dados à visualização do mapa, as ferramentas de mapa ficam disponíveis.

> [!NOTE]
> Este tutorial mostra apenas as funcionalidades e as opções disponíveis na versão **Standard** do ArcGIS Maps for Power BI. Se tiver sessão iniciada numa [conta do ArcGIS](https://doc.arcgis.com/en/maps-for-powerbi/get-started/account-types.htm) válida com o licenciamento adequado, terá acesso a mais funcionalidades.
...
> [!TIP]
> A Esri disponibiliza [ documentação completa](https://go.microsoft.com/fwlink/?LinkID=828772) sobre a utilização do ArcGIS Maps for Power BI.
...
Para obter mais informações, veja [Alterar o mapa base](https://doc.arcgis.com/en/maps-for-powerbi/design/change-the-basemap.htm) na ajuda online do ArcGIS Maps for Power BI.

#### <a name="show-map-layers"></a>Mostrar camadas do mapa

No ArcGIS Maps for Power BI, trabalha com dados geográficos através de camadas. As camadas são coleções lógicas de dados geográficos que são utilizadas para criar mapas. Também são a base para a análise geográfica.
...
Para obter mais informações, veja [Trabalhar com camadas](https://doc.arcgis.com/en/maps-for-powerbi/design/work-with-layers.htm) na ajuda online do ArcGIS Maps for Power BI.

#### <a name="specify-the-location-type"></a>Especificar o tipo de localização

O ArcGIS Maps for Power BI utiliza a tecnologia de ativação geográfica potente para posicionar com precisão os dados de localização no mapa e compõe automaticamente os itens como pontos ou limites por predefinição, com base no tipo de dados. Por exemplo, uma coordenada de latitude-longitude é composta como um ponto no mapa, enquanto um campo de estado é composto como um limite ou polígono. Pode alterar a forma como as localizações são compostas ao especificar o tipo de localização.
...
Para obter informações detalhadas, veja [Alterar o tema do mapa](https://doc.arcgis.com/en/maps-for-powerbi/design/change-the-map-theme.htm) na ajuda online do ArcGIS Maps for Power BI.
...
O ArcGIS Maps for Power BI proporciona várias ferramentas que lhe permitem descobrir padrões nos dados e contar uma história apelativa com um mapa.
...
O ArcGIS Maps for Power BI disponibiliza várias ferramentas de seleção diferentes para o ajudar a selecionar localizações no mapa. São apresentadas diferentes ferramentas, dependendo do tipo de camada atualmente selecionada na lista Camadas.
...
Para obter mais informações, veja [Selecionar localizações num mapa](https://doc.arcgis.com/en/maps-for-powerbi/design/select-features-on-the-map.htm) na ajuda online do ArcGIS Maps for Power BI.
...
> [!NOTE]
> A funcionalidade **Pesquisar** substitui a funcionalidade Alfinetes anteriormente disponível no ArcGIS Maps for Power BI.

Para obter mais informações, veja [Afixar uma localização](https://doc.arcgis.com/en/maps-for-powerbi/design/pin-locations.htm) na ajuda online do ArcGIS Maps for Power BI.
...
A conta do ArcGIS Maps for Power BI Standard inclui uma seleção de camadas demográficas dos EUA que ajudam a contextualizar os dados do Power BI.
...
As camadas de referência de dados demográficos são interativas; clique numa área para ver uma descrição com mais informações. Também pode utilizar a ferramenta **Seleção de referências** para [selecionar áreas no mapa](https://doc.arcgis.com/en/maps-for-powerbi/design/select-features-on-the-map.htm) que se enquadram numa área na camada de referência.

##### <a name="add-a-reference-layer-from-arcgis"></a>Adicionar uma camada de referência a partir do ArcGIS

O ArcGIS Maps for Power BI permite-lhe procurar camadas de funcionalidades partilhadas publicamente no ArcGIS e dá-lhe acesso a milhares de camadas de referência que disponibilizam informações sobre as áreas em torno das localizações na camada de dados. Quando inicia sessão no ArcGIS Maps for Power BI com uma conta válida do ArcGIS, pode procurar conteúdo na sua organização, nos grupos da sua organização ou no conteúdo público. Neste exemplo, como está a utilizar a conta Standard, apenas é apresentado o conteúdo partilhado publicamente nos resultados da pesquisa.
...
As camadas de referência do ArcGIS são interativas; clique numa área para ver uma descrição com mais informações. Também pode utilizar a ferramenta **Seleção de referências** para [selecionar áreas no mapa](https://doc.arcgis.com/en/maps-for-powerbi/design/select-features-on-the-map.htm) que se enquadram numa área na camada de referência.
...
Para obter mais informações, veja [Encontrar localizações próximas](https://doc.arcgis.com/en/maps-for-powerbi/design/find-nearby-locations.htm) na ajuda online do ArcGIS Maps for Power BI.

#### <a name="add-infographics-cards"></a>Adicionar cartões de infográficos

Tal como as camadas de referência, os cartões infográficos disponibilizam detalhes adicionais sobre as áreas no mapa. Os cartões de infográficos são colocados sobre o mapa e apresentam informações demográficas específicas sobre a área imediatamente ao redor de uma localização selecionada ou sobre a área geral apresentada na extensão do mapa atual. Para obter informações detalhadas, veja [Adicionar cartões de infográficos](https://doc.arcgis.com/en/maps-for-powerbi/design/add-infographics.htm) na ajuda online do ArcGIS Maps for Power BI.

### <a name="get-help"></a>Obter ajuda

A Esri disponibiliza documentação online completa sobre o ArcGIS Maps for Power BI.

Para aceder à ajuda online do ArcGIS Maps for Power BI a partir da visualização, siga estes passos:
...
3. Clique em **OK** na janela de confirmação apresentada.
    ...
    - Encontre respostas para [perguntas mais frequentes](https://doc.arcgis.com/en/maps-for-powerbi/get-started/pbi-faq.htm#anchor5) na ajuda online do ArcGIS Maps for Power BI.
...
### <a name="manage-the-use-of-arcgis-maps-for-power-bi-within-your-organization"></a>Gerir a utilização do ArcGIS Maps for Power BI na sua organização

O Power BI permite que os designers, administradores do Power BI e administradores de utilizadores façam a gestão da utilização do ArcGIS Maps for Power BI. As secções abaixo descrevem as ações que cada função pode assumir.
...
![Opções para designers](media/power-bi-visualizations-arcgis/arcgis-tutorial-17.png)

#### <a name="administrator-options"></a>Opções para administradores

No serviço do Power BI, os administradores podem desativar o ArcGIS Maps for Power BI para todos os utilizadores. Selecione **Definições** > **Portal de Administração** > **Definições de inquilino**. Quando for desativado, o Power BI deixará de apresentar o ícone do ArcGIS Maps for Power BI no painel de visualizações.

![Opções para administradores](media/power-bi-visualizations-arcgis/arcgis-tutorial-18.png)
...
| Power BI Report Server | Não |

\*Em ambientes móveis, pode apresentar mapas criados através da visualização do ArcGIS Maps for Power BI incluída com o Power BI ([conta Standard](https://doc.arcgis.com/en/maps-for-powerbi/get-started/account-types.htm)). Os mapas com conteúdo premium do ArcGIS não são suportados em ambientes móveis.
...
- [Interagir com um mapa ArcGIS que foi partilhado consigo](https://doc.arcgis.com/en/maps-for-powerbi/use/explore-maps.htm)
- Atualizações de [Publicações de blogues que anunciam os mapas do ArcGIS para o Power BI](https://www.esri.com/arcgis-blog/?s=#ArcGIS%20Maps%20for%20Power%20BI)
