...
Utilize o formato [SAML (Security Assertion Markup Language)](https://www.onelogin.com/pages/saml) para permitir uma conectividade de início de sessão único totalmente integrado. Ativar o SSO facilita a atualização de dados de origens no local em relatórios e dashboards do Power BI, ao mesmo tempo que respeita as permissões ao nível do utilizador configuradas nestas origens.
...
Suportamos origens de dados adicionais (incluindo HANA) com o [Kerberos](service-gateway-sso-kerberos.md).
...
1. Crie o certificado X509 e chave privada da AC de Raiz. Por exemplo, para criar o certificado X509 e a chave privada da AC de Raiz no formato .pem:
   ...
   ```
    ...
    Certifique-se de que a chave privada da AC de Raiz está devidamente protegida, uma vez que, se for obtida por terceiros, poderá ser utilizada para ter acesso não autorizado ao servidor HANA.
...
1. Converta o certificado no formato pfx ao executar o seguinte comando. Tenha em atenção que este comando atribui o nome samlcert.pfx ao ficheiro .pfx resultante e define "root" como a respetiva palavra-passe.
...
1. Abra o **PowerBI.DataMovement.Pipeline.GatewayCore.dll.config** e localize a secção *SapHanaSAMLCertThumbprint*. Cole o thumbprint que copiou.
