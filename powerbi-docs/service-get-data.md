...
Neste artigo, vamos falar sobre os diferentes tipos de origens de dados aos quais se pode ligar utilizando o serviço Power BI. Tenha em consideração que há muitos outros tipos de origens de dados a aprtir dos quais pode obter dados. Mas estes podem exigir, primeiro, o uso do Power BI Desktop ou da consulta avançada de dados e recursos de modelação do Excel. Entraremos em mais detalhes posteriormente. Por enquanto, vamos examinar os diferentes tipos de origens de dados aos quais se pode ligar diretamente a partir do site do serviço Power BI.
...
O local onde armazena os ficheiros também faz muita diferença. O OneDrive para Empresas fornece maior nível de flexibilidade e integração com o Power BI. Se mantiver os ficheiros na unidade local não há problemas, mas se precisar de atualizar os dados, serão necessários alguns passos adicionais. Serão fornecidos mais detalhes nos artigos ligados.
...
**Organizacional** – se o utilizador e outros utilizador da organização tiver numa conta do Power BI Pro, podem criar, partilhar e usar pacotes de conteúdo. Para saber mais, veja [Pacotes de conteúdo organizacional](service-organizational-content-pack-introduction.md).

## <a name="databases"></a>Bases de dados
![](media/service-get-data/pbi_getdata_databases.png)

**Bases de dados na cloud** – a partir do serviço Power BI, pode ligar-se à Base de Dados SQL do Azure, SQL Data Warehouse do Azure, Spark no Azure HD Insight e SQL Server Analysis Services utilizando o DirectQuery. As ligações do Power BI a estas bases de dados ocorrem em tempo real, ou seja, ao ligar-se, digamos, a uma Base de Dados SQL do Azure e começar a explorar os dados criando relatórios no Power BI, quando secciona os dados ou adiciona outro campo a uma visualização, é feita uma consulta diretamente na base de dados. Para saber mais, veja [Azure e Power BI](service-azure-and-power-bi.md).

**Bases de dados locais** – a partir do serviço Power BI, pode ligar-se diretamente às bases de dados de modelo de tabela do SQL Server Analysis Services. É necessário um gateway do Power BI Enterprise. Se não tiver a certeza sobre como ligar-se à base de dados de modelo de tabela da organização, consulte o administrador ou o departamento de TI. Para saber mais, veja [Dados de Tabela do SQL Server Analysis Services no Power BI](sql-server-analysis-services-tabular-data.md).

Para outros tipos de bases de dados na organização, será necessário, primeiro, usar o Power BI Desktop ou o Excel para ligar-se, consultar e carregar dados para um modelo de dados. Pode importar o ficheiro para o Power BI, onde um conjunto de dados é criado. Se configurar a atualização agendada, o Power BI usará as informações de ligação do ficheiro juntamente com as configurações de atualização definidas para ligar-se diretamente à origem de dados e consultar atualizações. Essas atualizações serão, então, carregadas no conjunto de dados no Power BI. Para saber mais, veja [Ligar-se a dados no Power BI Desktop](desktop-connect-to-data.md).
...
Se guardar os ficheiros no OneDrive para Empresas ou no SharePoint – Team Sites e se ligar ou importá-los para o Power BI, o conjunto de dados, relatórios e dashboard estarão sempre atualizados. Como o OneDrive e o Power BI estão na cloud, o Power BI pode ligar-se diretamente ao ficheiro guardado, uma vez por hora, e verificar se há atualizações. Se alguma for encontrada, o conjunto de dados e quaisquer visualizações são atualizadas automaticamente.

Os pacotes de conteúdo de serviços são atualizados automaticamente. Na maioria dos casos, uma vez por dia. Pode atualizar manualmente, mas se verá ou não os dados atualizados depende do fornecedor de serviço. Pacotes de conteúdo de outras pessoas na organização dependerão das origens de dados usadas e de como a pessoa que criou o pacote de conteúdo configurou a atualização.
...
O SQL Server Analysis Services é exclusivo no sentido em que se liga a este a partir do Power BI, trata-se de uma ligação em tempo real assim como uma base de dados do Azure na cloud, mas a base de dados em si está num servidor na organização. Este tipo de ligação requer um gateway do Power BI, geralmente configurado por um departamento de TI.
...
Para todas as origens de dados utilizadas no serviço Power BI, aplicam-se as seguintes considerações e limitações. Existem outras limitações que se aplicam a funcionalidades específicas, mas a lista seguinte aplica-se ao serviço Power BI geral:
