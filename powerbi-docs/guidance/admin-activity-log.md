...
- O utilizador do script do PowerShell tem de iniciar sessão através do [cmdlet Connect-PowerBIServiceAccount](/powershell/module/microsoftpowerbimgmt.profile/connect-powerbiserviceaccount) e introduzir as credenciais de administrador do inquilino do Power BI quando forem pedidas. São necessários privilégios de administrador inquilino para utilizar a API Registo de Atividades.

> [!IMPORTANT]
> Os utilizadores sem privilégios de administrador inquilino não poderão executar fragmentos de código no script de amostra.
