...
Quando inicia o relatório de métricas de utilização melhorado pela primeira vez, o Power BI cria automaticamente o conjunto de dados Relatório de Métricas de utilização. O relatório de métricas de utilização melhorado baseia-se nesse conjunto de dados. O Power BI atualiza este conjunto de dados diariamente. Apesar de não poder alterar o agendamento das atualizações, pode atualizar as credenciais que o Power BI utiliza para atualizar os dados das métricas de utilização. Poderá ter de retomar a atualização agendada caso as credenciais expirem ou se tiver removido o utilizador que iniciou pela primeira vez o relatório de métricas de utilização a partir da área de trabalho onde reside o conjunto de dados.
...
| --- | --- | --- |
| Comunicar utilização | Visualizações do relatório/o relatório é aberto | Sempre que alguém abre um relatório, é registada uma Visualização do Relatório que representa patamares exclusivos no relatório. Responde à pergunta: "Com que frequência se acede ao relatório?" Esta definição de uma Visualização do Relatório é diferente da dos relatórios de métricas de utilização anteriores. Alterar as páginas do relatório já não é considerado uma Visualização do Relatório adicional. Pelo contrário, a alteração das páginas do relatório é contabilizada para a próxima métrica, Visualizações das Páginas do Relatório. Atividades como partilhar e afixar já não são consideradas nas métricas de utilização. |
| Comunicar utilização | Visualizações das Páginas do Relatório | Sempre que alguém abre um relatório, é registada uma Visualização das Páginas do Relatório. Representa o total de visualizações em todas as páginas. Responde à pergunta: "Com que frequência se acede às páginas dos relatórios?" Pelo contrário, a alteração das páginas do relatório é contabilizada para a próxima métrica, Visualizações das Páginas do Relatório. Veja [Considerações e Limitações](#considerations-and-limitations) para obter detalhes importantes. |
...
| Comunicar utilização | Visualizações do relatório por dia | Número total de Visualizações do Relatório por dia. Contabilizado ao nível do relatório, não contempla as Visualizações das Páginas do Relatório. |
...
| Comunicar utilização | Páginas | Se o relatório tem mais de 1 página, segmente o relatório pela(s) página (s) que foi(ram) visualizada(s). "Em branco" significa que foi adicionada uma página de relatório no prazo de 24 horas após a nova página aparecer na lista de segmentação ou as páginas de relatório foram eliminadas. "Em branco" captura estes tipos de situações. |
...
Por predefinição, os dados por utilizador são ativados para as métricas de utilização. As informações da conta do consumidor de conteúdos estão incluídas no relatório de métricas. Se os administradores não quiserem expor estas informações a alguns ou a todos os utilizadores, poderão excluir as informações dos utilizadores do relatório de utilização. Nas definições de inquilino do portal de administração do Power BI, podem desativar os dados por utilizador nas métricas de utilização para grupos de segurança específicos ou para toda a organização.
