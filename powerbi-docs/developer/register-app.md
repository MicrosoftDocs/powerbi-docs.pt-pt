...
1. Aceda a [dev.powerbi.com/apps](https://dev.powerbi.com/apps).
...
4. No painel de navegação esquerdo, em **Todos os serviços**, **Azure Active Directory**, selecione **Registos das Aplicações** e, em seguida, selecione **Novo registo de aplicação**.
...
2. Selecione **Permissões precisas** em **Acesso à API**.
    ...
    ![Permissões obrigatórias de aplicações do Azure AD](media/register-app/powerbi-embedded-azuread-app-required-permissions.png)
...
4. Selecione todas as permissões em **Permissões Delegadas**. Selecione-as uma a uma para guardar as seleções. Selecione **Guardar** quando terminar.
    ...
    ![Permissões de aplicação 04](media/register-app/powerbi-embedded-azuread-app-permissions04.png)
...
1. Tem de obter os principais de serviço existentes (utilizadores) no seu inquilino. Para obter informações sobre como fazê-lo, veja [Obter servicePrincipal](https://developer.microsoft.com/graph/docs/api-reference/beta/api/serviceprincipal_get).
...
4. Conceder Permissões de Aplicação à API do Power BI
   ...
   * **Principal** serve para conceder permissões em nome de um utilizador específico. Neste caso, deve ser adicionada uma propriedade adicional ao corpo do pedido: *principalId = {User_ObjectId}*.
    ...
    O **resourceId** *c78a3685-1ce7-52cd-95f7-dc5aea8ec98e* não é universal mas está dependente de inquilinos. Este valor é o objectId da aplicação "Power BI Service" no inquilino do Azure Active Directory (AAD).
