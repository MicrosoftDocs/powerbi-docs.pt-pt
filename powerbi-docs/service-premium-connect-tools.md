...
O Power BI Desktop pode ligar-se a um conjunto de dados do Power BI Premium como se fosse uma base de dados de modelos implementada no Azure Analysis Services ou no SQL Server Analysis Services. Neste caso, o Power BI Desktop utiliza o ponto final XMLA. No entanto, recomenda-se que os utilizadores do Power BI Desktop utilizem, em vez disso, a funcionalidade de ligação Em direto criada especificamente para conjuntos de dados do Power BI. A utilização da ligação Em direto proporciona uma experiência de descoberta melhorada que mostra o nível de recomendação dos conjuntos de dados, e os utilizadores não precisam de controlar os URLs da área de trabalho; podem simplesmente escrever o nome do conjunto de dados. Para obter mais informações, veja [Ligar a conjuntos de dados no serviço Power BI a partir do Power BI Desktop](desktop-report-lifecycle-datasets.md).
